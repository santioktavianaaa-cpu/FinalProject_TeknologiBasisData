const API_URL = 'http://127.0.0.1:5000/api/sales/employee';
let allData = [];
    ...
    data.forEach(item => {
        html += `
            <tr>
                ...
                <td style="text-align: center;">${formatNumber(item.jumlahTransaksi)}</td>
                ...
                <td style="text-align: right;">${formatCurrency(item.rataRataTransaksi)}</td>
            </tr>
        `;
    });
...
async function loadEmployee() {
    try {
        ...
        document.getElementById('searchInfo').textContent = 
            `Menampilkan semua data (${allData.length} pegawai)`;
    } catch (error) {
